...
function setSpeed(s) {
  // Mode x0.1 : affichage x0.1 mais vitesse réelle x0.02
  if (s === 0.1) {
    gameSpeed = 0.02;
  } else {
    gameSpeed = s;
  ...
  const map = { 0.1: 'spd05', 1: 'spd1', 2: 'spd2', 4: 'spd4' };
...
function initGame() {
  ...
  // Reset module unlocks (only lock modules that are not unlocked by default in config)
  Object.keys(MODULE_TYPES).forEach(k => {
    if (!MODULE_TYPES[k].unlocked) MODULE_TYPES[k].unlocked = false;
  });
...
// PAUSE SYSTEM
// ============================================================
function togglePause(force, defaultPane) {
  if (!G || G.over || !gameStarted) return;
  if (G.directiveOpen) return;
  const overlay = document.getElementById('pauseOverlay');
  if (!overlay) return;

  const willBeActive = force === undefined ? !overlay.classList.contains('active') : !!force;
  G.paused = willBeActive;

  if (willBeActive) {
    overlay.classList.add('active');
    if (typeof showSettingsPane === 'function') showSettingsPane(defaultPane || 'pause');
    if (typeof renderGraphicsSettings === 'function') renderGraphicsSettings();
    if (typeof renderBalanceSettings === 'function') renderBalanceSettings();
  } else {
