...
function getWaveEnemyCount(wave, effects) {
  ...
  // Wave 10 = ~95 enemies: roughly the old wave 30, then controlled linear growth.
  const base = 10 + wave * 6 + earlyCurve * earlyCurve * 0.25 + Math.max(0, wave - 10) * 2;
  return Math.max(8, Math.round(base * (BALANCE.wavePopulation || 1) * ((effects && effects.countMult) || 1)));
}

function getWaveFrontCount(wave, effects) {
  const milestones = wave <= 2 ? 1 : wave <= 5 ? 2 : wave <= 8 ? 3 : wave <= 12 ? 4 :
    wave <= 16 ? 5 : wave <= 21 ? 6 : wave <= 27 ? 7 : wave <= 34 ? 8 : wave <= 42 ? 9 : 10;
  return Math.max(1, Math.min(10, Math.round(milestones * (BALANCE.frontIntensity || 1)) + ((effects && effects.extraCones) || 0)));
}
