...
function recalcEnergy() {
  ...
  const relayDiscountByCell = new Map();
  for (const relay of G.modules) {
    ...
    const discount = (relayDef.adjacencyEnergyDiscount || 0) * (1 + (relay.level - 1) * .025);
    for (const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]) {
      const key = (relay.gx + dx) + ',' + (relay.gy + dy);
      relayDiscountByCell.set(key, (relayDiscountByCell.get(key) || 0) + discount);
    }
  }
  ...
  G.modules.forEach(m => {
    ...
    if (def.isReactor) {
      ...
      base += Math.min(150, Math.abs(def.energy) * supportLvMult * mkMult) * upgEnergyBonus;
    }
    if (def.baseEnergy) base += def.baseEnergy * (1 + (m.level - 1) * .08) * (1 + (m.mk - 1) * .25);
    if (def.energy > 0) {
      const discount = relayDiscountByCell.get(m.gx + ',' + m.gy) || 0;
      const policyUse = G.energyPolicy === 'eco' ? .82 : G.energyPolicy === 'assault' ? 1.18 : 1;
      used += def.energy * Math.max(.45, 1 - Math.min(.55, discount)) * policyUse;
    }
  });
