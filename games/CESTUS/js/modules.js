...
function computeModuleStats(mod) {
  ...
  let aura_dmg=0, aura_fire=0, aura_hp=0, aura_credits=0, aura_samples=0, aura_range=0, aura_accuracy=0;
  const adjacent = [];
  if (G._moduleCellIndex instanceof Map) {
    for (const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]) {
      const nearby = G._moduleCellIndex.get((mod.gx + dx) + ',' + (mod.gy + dy));
      if (nearby?.alive) adjacent.push(nearby);
    }
  } else {
    for (const nearby of G.modules) if (nearby.alive && nearby !== mod && isAdjacent(mod,nearby)) adjacent.push(nearby);
  }
  adjacent.forEach(m2 => {
    ...
    if (def2.isRangeBoost) {
      ...
      aura_accuracy += (def2.aura?.accuracy || 0) * adjLvMult;
    }
  });

  aura_dmg = Math.min(.75, aura_dmg); aura_fire = Math.min(.8, aura_fire);
  aura_hp = Math.min(1.5, aura_hp); aura_range = Math.min(.6, aura_range);
  aura_credits = Math.min(.75, aura_credits); aura_samples = Math.min(.5, aura_samples);
  aura_accuracy = Math.min(.75, aura_accuracy);
  ...
  const energyFactor = energyRatio <= 1 ? 1 + Math.min(.12, (1 - energyRatio) * .12) : 1;
  let penaltyMult = 1;
  if (energyRatio > 1) {
    const penaltyPercent = energyRatio - 1;
    penaltyMult = Math.max(0.22, 1 - penaltyPercent * .72);
  }

  const policyPower = G.energyPolicy === 'assault' ? 1.10 : G.energyPolicy === 'eco' ? .90 : 1;

  const dmgMult = lvlMult * mkMult * (1 + upg.dmg + aura_dmg + overclock * 0.5) * (1 + sp.dmg * 0.01) * energyFactor * policyPower * penaltyMult;
  const fireLvlMult = 1 + (mod.level - 1) * 0.02;
  const fireMult = fireLvlMult * (1 + upg.fireRate + aura_fire + overclock) * (1 + sp.speed * 0.01) * energyFactor * policyPower * penaltyMult;
  ...
  return {
    ...
    accuracy: 1 - Math.max(0, (def.spread || .035) * Math.max(.12, 1 - aura_accuracy)),
  ...
  if (now - G.lastRegenTick > 1000) {
    ...
    G.modules.forEach(m => {
      ...
      if (stats.healRate > 0) {
        ...
        forEachModuleInRange(m.x, m.y, G.CELL * 1.5, m2 => {
