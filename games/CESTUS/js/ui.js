...
function updateHUD() {
  ...
  const production = G.totalEnergyProduction || 0;
  const consumption = G.totalEnergyConsumption || 0;
  const surplus = production - consumption;
  setTextIfChanged(el, `${Math.round(consumption)} / ${Math.round(production)}`);
  el.style.color = consumption > production ? 'var(--neon-red)' : consumption >= production * 0.9 ? 'var(--neon-orange)' : 'var(--neon-yellow)';

  // Oil HUD
  const oilEl = hudEl('hudOil');
  if (oilEl) {
    const oilProd = G.totalOilProduction || 0;
    const oilCons = G.totalOilConsumption || 0;
    setTextIfChanged(oilEl, `${Math.round(oilCons)} / ${Math.round(oilProd)}`);
    oilEl.style.color = oilCons > oilProd ? 'var(--neon-red)' : oilCons >= oilProd * 0.9 ? 'var(--neon-orange)' : 'var(--neon-amber)';
  }
...
function activateMainTab(element, tab) {
  const phone = document.documentElement.dataset.mobileDevice === 'true';
  const panel = document.getElementById('bottomPanel');
  const wasActive = element?.classList.contains('active');
  if (phone && typeof setMobilePanelExpanded === 'function') {
    if (wasActive && panel && !panel.classList.contains('mobile-collapsed')) {
...
function renderTab(tab) {
  ...
  if (tab === 'shop') {
    ...
    shopEntries.forEach(([typeId, def]) => {
      ...
      let energyStr = '';
      if (def.energyProduction) energyStr = `+${def.energyProduction}⚡`;
      else if (def.energyConsumption) energyStr = `-${def.energyConsumption}⚡`;
      else energyStr = '0⚡';
      
      let oilStr = '';
      if (def.oilProduction) oilStr = `+${def.oilProduction}●`;
      else if (def.oilConsumption) oilStr = `-${def.oilConsumption}●`;
      
      const resourceStr = oilStr ? `${energyStr} ${oilStr}` : energyStr;
      ...
      html += `<div class="shop-card ${cls}" data-search="${searchText}" onclick="handleShopClick('${typeId}')">
        ...
        <div class="card-meta"><span>${resourceStr || '0⚡'}</span><span>×${placedCount}</span></div>
    ...
    // Attach long-press listeners for mobile multi-placement
    if (typeof attachShopCardLongPress === 'function') {
      const phone = document.documentElement.dataset.mobileDevice === 'true';
      if (phone) {
        shopEntries.forEach(([typeId, def]) => {
          if (!def.unlocked) return;
          const cards = content.querySelectorAll(`.shop-card[onclick="handleShopClick('${typeId}')"]`);
          cards.forEach(card => attachShopCardLongPress(card, typeId));
        });
      }
    }
  ...
  // Sync multi-place stop button visibility
...
function updateModuleInfo(mod) {
  ...
  if (def.energyProduction) {
    extra += `<div class="info-stat"><span class="info-stat-label">Production</span><span class="info-stat-value" style="color:var(--neon-green)">+${def.energyProduction} ⚡/s</span></div>`;
  }
  if (def.energyConsumption) {
    extra += `<div class="info-stat"><span class="info-stat-label">Consommation</span><span class="info-stat-value" style="color:var(--neon-orange)">-${def.energyConsumption} ⚡/s</span></div>`;
  }
  if (def.oilProduction) {
    extra += `<div class="info-stat"><span class="info-stat-label">Pétrole</span><span class="info-stat-value" style="color:var(--neon-amber)">+${def.oilProduction} ●/s</span></div>`;
  }
  if (def.oilConsumption) {
    extra += `<div class="info-stat"><span class="info-stat-label">Pétrole</span><span class="info-stat-value" style="color:var(--neon-red)">-${def.oilConsumption} ●/s</span></div>`;
  }
  if (def.energyCapacity) {
    extra += `<div class="info-stat"><span class="info-stat-label">Stockage</span><span class="info-stat-value" style="color:var(--neon-cyan)">${def.energyCapacity} ⚡</span></div>`;
  }
  if (def.energyBoost) {
    extra += `<div class="info-stat"><span class="info-stat-label">Boost</span><span class="info-stat-value" style="color:var(--neon-purple)">+${Math.round(def.energyBoost * 100)}%</span></div>`;
  }
  if (def.aura) {
...
function showNotif(msg, cls) {
  ...
  el.textContent = msg;
  if (document.documentElement.dataset.mobileDevice === 'true') container.replaceChildren();
...
function getEnemyRole(cfg) {
  if (cfg.isFortress) return 'FORTERESSE';
  ...
  if (cfg.siegeCapable) return 'ARTILLEUR';
