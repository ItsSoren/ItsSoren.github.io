  ...
  if (tab === 'shop') {
    ...
    if (G.activeShopTab === 'offensive') {
      ...
      doctrines.forEach(([id,label]) => {
        html += `<button class="doctrine-pill ${G.activeShopGroup === id ? 'active' : ''}" onmousedown="G.activeShopGroup='${id}'; renderTabs()">${label}</button>`;
      });
      html += '</div>';
    }
    html += `<div class="card-grid ${G.activeShopTab === 'offensive' ? 'with-doctrines' : ''}">`;
    
    Object.entries(MODULE_TYPES).forEach(([typeId, def]) => {
      if (def.category !== G.activeShopTab || typeId === 'core') return;
      if (G.activeShopTab === 'offensive' && G.activeShopGroup !== 'all' && def.group !== G.activeShopGroup) return;
