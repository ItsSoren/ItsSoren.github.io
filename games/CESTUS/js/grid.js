...
function isInsideCircle(gx, gy) {
  ...
  return cx * cx + cy * cy <= G.GRID_R * G.GRID_R;
}
...
function canPlaceModule(gx, gy) {
  ...
  const def = G.placingModule ? MODULE_TYPES[G.placingModule] : null;
  if (def && (def.canPlaceAnywhere || def.isSecondaryCore)) return true;
  // Modules can be placed adjacent to: main core, other modules, OR secondary cores
