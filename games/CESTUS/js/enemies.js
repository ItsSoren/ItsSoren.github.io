...
function spawnEnemy(typeKey, isBossOverride, spawnX, spawnY, riftDirective, waveId, bossRankOverride) {
  ...
  const strength = 1 + (strengthWave - 1) * 0.25;
    ...
    if (e.shootRange > 0 && e.target) {
      ...
      if (e.siegeCapable) {
        if (e.siegeMode && (now >= e.siegeUntil || !inSiegeBand)) {
          e.siegeMode = false;
          e.siegeCooldownUntil = now + 5000 + Math.random() * 5000;
          spawnParticle(e.x,e.y,e.color,5);
        } else if (!e.siegeMode && inSiegeBand && now >= e.siegeCooldownUntil) {
          e.siegeMode = true;
          e.siegeUntil = now + 5000 + Math.random() * 4000;
          spawnParticle(e.x,e.y,'#ffffff',8);
        }
