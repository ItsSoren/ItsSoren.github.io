...
Object.assign(MODULE_TYPES, {
  prism: {
    name: 'PRISME DÉSIGNATEUR', icon: '◇', color: '#62f6ff', energy: 14,
    ...
    desc: 'Marque la cible : chaque attaque suivante inflige +8% de dégâts.',
    unlocked: false, cost: { credits: 230, samples: 1 }, category: 'offensive', unlockReq: { credits: 200, samples: 1 }
  },
  null_lance: {
    name: 'LANCE DU VIDE', icon: '↦', color: '#c4b5fd', energy: 28,
    ...
    desc: 'Traverse les lignes et ignore blindage, protection et réduction.',
    unlocked: false, cost: { credits: 560, samples: 5 }, category: 'offensive', unlockReq: { samples: 6 }
  },
  executioner: {
    name: 'EXÉCUTEUR', icon: '⌁', color: '#ff6b7a', energy: 22,
    ...
    desc: 'Inflige ×4 aux ennemis sous 25% de leurs points de vie.',
    unlocked: false, cost: { credits: 390, samples: 3 }, category: 'offensive', unlockReq: { samples: 4 }
  },
  acid_weaver: {
    name: 'TISSE-ACIDE', icon: '≋', color: '#b7f34a', energy: 17,
    ...
    desc: 'Ronge 8% de blindage par impact et applique une corrosion persistante.',
    unlocked: false, cost: { credits: 310, samples: 2 }, category: 'offensive', unlockReq: { samples: 3 }
  },
  chrono_anchor: {
    name: 'ANCRE CHRONO', icon: '◷', color: '#72ddf7', energy: 32,
    ...
    desc: 'Une impulsion circulaire fige presque tous les ennemis proches.',
    unlocked: false, cost: { credits: 480, samples: 4 }, category: 'offensive', unlockReq: { samples: 5 }
  },
  emp_spire: {
    name: 'FLÈCHE EMP', icon: '⌁', color: '#69f0d1', energy: 24,
    ...
    desc: 'Neutralise téléportation, soin, invocation et capacités de cinq cibles.',
    unlocked: false, cost: { credits: 420, samples: 3 }, category: 'offensive', unlockReq: { samples: 4 }
  },
  mine_architect: {
    name: 'ARCHITECTE MINIER', icon: '⌾', color: '#ffb84d', energy: 16,
    ...
    desc: 'Déploie des mines intelligentes persistantes sur les routes adverses.',
    unlocked: false, cost: { credits: 340, samples: 2 }, category: 'offensive', unlockReq: { credits: 320, samples: 2 }
  },
  cluster_bloom: {
    name: 'FLORA CLUSTER', icon: '✣', color: '#ff8b5c', energy: 26,
  ...
  nova_seed: {
    ...
    desc: 'Les ennemis éliminés explosent et propagent une onde létale.',
    unlocked: false, cost: { credits: 370, samples: 3 }, category: 'offensive', unlockReq: { samples: 4 }
  },
  kinetic_battery: {
    name: 'BATTERIE KINÉTIQUE', icon: '▰', color: '#f6d365', energy: 18,
    ...
    desc: 'Accumule cinq tirs puis libère un sixième impact sept fois plus puissant.',
    unlocked: false, cost: { credits: 330, samples: 2 }, category: 'offensive', unlockReq: { credits: 300, samples: 2 }
  },
  shardstorm: {
    name: 'TEMPÊTE D’ÉCLATS', icon: '≪', color: '#9ae6b4', energy: 27,
    ...
    desc: 'Projette sept éclats perforants dans un large cône.',
    unlocked: false, cost: { credits: 410, samples: 4 }, category: 'offensive', unlockReq: { samples: 5 }
  },
  aurora_array: {
    name: 'MATRICE AURORE', icon: '≋', color: '#7ee8fa', energy: 29,
  ...
  phase_repeater: {
    ...
    desc: 'Flux rapide qui traverse toutes les défenses physiques.',
    unlocked: false, cost: { credits: 620, samples: 7 }, category: 'offensive', unlockReq: { samples: 8 }
  },
  gravity_hammer: {
    name: 'MARTEAU GRAVITON', icon: '◉', color: '#f0abfc', energy: 38,
    ...
    desc: 'Impact de zone massif qui projette toute une escouade en arrière.',
  ...
  capacitor_bank: {
    name:'BANQUE CAPACITIVE', icon:'▥', color:'#ffe082', energy:0, hp:190, isPassive:true,
  ...
  grid_relay: {
    name:'RELAIS DE GRILLE', icon:'⌬', color:'#67e8f9', energy:4, hp:140, isPassive:true,isEnergyRelay:true,
    adjacencyEnergyDiscount:.22, mechanic:'ÉCO ADJACENTE', shortDesc:'-22% énergie adjacente.',
    desc:'Réduit de 22% la consommation énergétique de chaque module directement adjacent.',
    unlocked:false,cost:{credits:310,samples:3},category:'support',unlockReq:{samples:3}
  },
  repair_forge: {
    name:'FORGE NANO',icon:'✚',color:'#86efac',energy:18,hp:180,isPassive:true,isRegen:true,healRate:5,
    mechanic:'RÉPARATION',shortDesc:'Réparation adjacente rapide.',
    desc:'Répare fortement les structures adjacentes et stabilise les lignes exposées.',
    unlocked:false,cost:{credits:340,samples:3},category:'support',unlockReq:{samples:3}
  },
  data_foundry: {
    name:'FONDERIE DE DONNÉES',icon:'¢',color:'#fde047',energy:24,hp:95,isPassive:true,isHarvester:true,passiveCredits:12,
    mechanic:'RENDEMENT',shortDesc:'+12 crédits par seconde.',
    desc:'Transforme le surplus énergétique en crédits continus, sans dépendre des éliminations.',
    unlocked:false,cost:{credits:420,samples:4},category:'support',unlockReq:{samples:4}
  },
  sample_lab: {
    name:'LABORATOIRE RIFT',icon:'⚗',color:'#d8b4fe',energy:42,hp:90,isPassive:true,isReplicator:true,replicatorBaseSec:28,
    mechanic:'SYNTHÈSE',shortDesc:'1 échantillon toutes les 28s.',
    desc:'Condense les résidus de faille et synthétise régulièrement un échantillon.',
    unlocked:false,cost:{credits:520,samples:5},category:'support',unlockReq:{samples:6}
  },
  targeting_matrix: {
    name:'MATRICE DE VISÉE',icon:'⌖',color:'#5eead4',energy:20,hp:110,isPassive:true,isRangeBoost:true,
    aura:{range:.10,accuracy:.32},mechanic:'PRÉCISION',shortDesc:'Portée +10%, dispersion -32%.',
    desc:'Améliore la portée et resserre fortement la dispersion des tourelles adjacentes.',
    unlocked:false,cost:{credits:390,samples:4},category:'support',unlockReq:{samples:4}
  },
  coolant_loop: {
    name:'BOUCLE CRYO',icon:'❅',color:'#7dd3fc',energy:16,hp:125,isPassive:true,isAmplifier:true,
    aura:{dmg:0,fireRate:.18},mechanic:'REFROIDISSEMENT',shortDesc:'Cadence adjacente +18%.',
    desc:'Dissipe la chaleur des armes adjacentes et améliore leur cadence sans augmenter les dégâts.',
    unlocked:false,cost:{credits:360,samples:3},category:'support',unlockReq:{samples:3}
  },
  fortress_node: {
    name:'NŒUD FORTERESSE',icon:'⬢',color:'#93c5fd',energy:28,hp:420,isPassive:true,isShield:true,
    aura:{hp:.55},mechanic:'BASTION',shortDesc:'PV adjacents +55%.',
    desc:'Projette une enveloppe structurelle massive autour des modules adjacents.',
    unlocked:false,cost:{credits:480,samples:5},category:'support',unlockReq:{samples:5}
  },
  salvage_array: {
    name:'MATRICE DE RÉCUP',icon:'♲',color:'#f0abfc',energy:25,hp:115,isPassive:true,isCollector:true,
    aura:{credits:.25,samples:.12},mechanic:'SALVAGE',shortDesc:'Butin adjacent amélioré.',
    desc:'Analyse les cibles détruites par les armes adjacentes et augmente leur butin.',
...
Object.assign(ENEMY_TYPES, {
  ...
  siegeCrawler: {
    ...
    shootRange:520,shootRate:2200,shootDmg:34,siegeCapable:true,siegeMinRange:250
  }
});

ENEMY_TYPES.ranged.siegeCapable = true;
ENEMY_TYPES.ranged.siegeMinRange = 125;
ENEMY_TYPES.sniper.siegeCapable = true;
ENEMY_TYPES.sniper.siegeMinRange = 220;

WAVE_SPAWN_WEIGHTS.mid.push(
  {type:'regenerator',w:0.06}, {type:'phaser',w:0.05}, {type:'siphon',w:0.04}
);
WAVE_SPAWN_WEIGHTS.late.push(
  {type:'regenerator',w:0.09}, {type:'phaser',w:0.08}, {type:'commander',w:0.07},
  {type:'siphon',w:0.06}, {type:'berserker',w:0.08}, {type:'juggernaut',w:0.05}, {type:'siegeCrawler',w:0.07}
);
