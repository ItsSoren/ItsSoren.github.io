...
function drawModules(now) {
  ...
  G.modules.forEach(mod => {
    ...
    ctx.beginPath();
    if (def.isSecondaryCore) {
      // Secondary core looks like main core but smaller and with a distinct ring
      drawHexagon(ctx, x, y, size * 0.95);
      ctx.fill();
      ctx.stroke();
      // Distinctive secondary ring
      ctx.save();
      ctx.translate(x,y);
      ctx.rotate(now * 0.001);
      ctx.globalCompositeOperation = 'lighter';
      ctx.globalAlpha = 0.55 + Math.sin(now * 0.004) * 0.15;
      ...
      ctx.fillText('2ND', 0, size * 1.58);
      ctx.restore();
    } else if (def.isWall) {
      // Shield wall — thick fortress block
      const bw = size * 1.6, bh = size * 1.6;
      ctx.rect(x - bw*0.5, y - bh*0.5, bw, bh);
      ctx.fill();
      ctx.stroke();
      // Cross bars
      ctx.save();
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(-bw*0.5+4, 0); ctx.lineTo(bw*0.5-4, 0);
      ctx.moveTo(0, -bh*0.5+4); ctx.lineTo(0, bh*0.5-4);
      ctx.stroke();
      ctx.restore();
    } else if (def.isCore) {
    ...
    if (!def.isSecondaryCore && !def.isWall) {
      ctx.fill();
      ctx.stroke();
    }
    ...
    if ((def.category === 'support' || def.isAmplifier || def.isShield || def.isCollector || def.isRangeBoost || def.isRegen || def.isPassive) && isSelected) {
      ctx.beginPath();
      ctx.arc(x, y, stats.range, 0, Math.PI * 2);
...
function drawEnemies() {
  ...
  for (let i = 0; i < G.enemies.length; i++) {
    ...
    if (e.isFortress) {
      // Fortress boss — epic cannon mounts & siege platform
      const ft = G.now * 0.001;
      ctx.save();ctx.translate(x,y);
      // Pulsing outer danger ring
      ctx.globalCompositeOperation='lighter';
      ctx.globalAlpha=0.18+Math.sin(ft*3)*0.07;
      ctx.strokeStyle='#ff2a5f';ctx.lineWidth=6;
      ctx.beginPath();ctx.arc(0,0,e.size+22,0,Math.PI*2);ctx.stroke();
      ctx.globalAlpha=0.1;
      ctx.strokeStyle='#ff2a5f';ctx.lineWidth=12;
      ctx.beginPath();ctx.arc(0,0,e.size+32,0,Math.PI*2);ctx.stroke();
      ctx.globalCompositeOperation='source-over';
      // Draw 4 heavy cannon barrels
      for(let ci=0;ci<4;ci++){
        const ca=ci*Math.PI/2+(ft*0.18);
        const cx2=Math.cos(ca)*e.size*.72, cy2=Math.sin(ca)*e.size*.72;
        ctx.save();ctx.translate(cx2,cy2);ctx.rotate(ca);
        // Barrel
        ctx.fillStyle='#991122';ctx.globalAlpha=1;
        ctx.fillRect(2,-4,e.size*.52,8);
        ctx.fillStyle='#cc2244';
        ctx.fillRect(2,-5,e.size*.52+4,10);
        // Muzzle
        ctx.fillStyle='#ff2a5f';
        ctx.beginPath();ctx.arc(e.size*.52+4,0,5+Math.sin(ft*8+ci)*1.2,0,Math.PI*2);ctx.fill();
        // Muzzle glow
        ctx.globalCompositeOperation='lighter';ctx.globalAlpha=0.4+Math.sin(ft*8+ci)*0.25;
        ctx.fillStyle='#ff2a5f';
        ctx.beginPath();ctx.arc(e.size*.52+4,0,10,0,Math.PI*2);ctx.fill();
        ctx.globalCompositeOperation='source-over';
        ctx.restore();
      }
      // Spinning danger chevrons
      ctx.globalAlpha=0.65;ctx.rotate(-ft*.4);
      for(let i=0;i<6;i++){
        const a=i*Math.PI/3;
        ctx.strokeStyle='#ff2a5f';ctx.lineWidth=2;
        ctx.beginPath();
        ctx.moveTo(Math.cos(a)*(e.size+8),Math.sin(a)*(e.size+8));
        ctx.lineTo(Math.cos(a+0.4)*(e.size+14),Math.sin(a+0.4)*(e.size+14));
        ctx.stroke();
      }
      ctx.restore();
      // Siege mode glow
      if(e.siegeMode){
        ...
        ctx.globalAlpha=0.15+Math.sin(G.now*.008)*0.08;
        ctx.fillStyle='#ff2a5f';
        ctx.beginPath();ctx.arc(0,0,e.shootRange||600,0,Math.PI*2);ctx.fill();
        ctx.restore();
      }
      ctx.font = '14px Orbitron, monospace';
      ctx.fillStyle = '#ff2a5f';
      ctx.textAlign = 'center';
      ctx.fillText('⚠ FORTERESSE', x, y - e.size - 28);
      ctx.font = '9px Orbitron, monospace';
      ctx.fillStyle = '#ff8aaa';
      ctx.fillText(e.siegeMode ? '[ MODE SIÈGE ]' : '[ EN APPROCHE ]', x, y - e.size - 16);
    } else if (e.isBoss) {
...
function drawProjectileBeam(p, style) {
  ...
  // Laser propagation animation over first frames
  const maxLife = p.maxBeamLife || 8;
  const progress = p.beamProgress !== undefined ? p.beamProgress : Math.min(1, ((maxLife - p.beamLife + 1) / 3));
  
  const targetX = p.x + (p.tx - p.x) * progress;
  const targetY = p.y + (p.ty - p.y) * progress;
  
  const dx=targetX-p.x, dy=targetY-p.y, len=Math.hypot(dx,dy)||1, nx=-dy/len, ny=dx/len;
  ctx.save();
  ctx.globalCompositeOperation='lighter';
  ctx.lineCap='round';
  const t = G.now * 0.004;
  ...
  const line = (offset,width,color,lineAlpha) => {
    ...
    ctx.beginPath();ctx.moveTo(p.x+nx*offset,p.y+ny*offset);ctx.lineTo(targetX+nx*offset,targetY+ny*offset);ctx.stroke();
  };

  if(style==='electric'){
    // Glow aura
    line(0, p.size*6, p.color, 0.08);
    line(0, p.size*3, p.color, 0.14);
    // Main animated lightning bolt — 3 staggered paths
    for(let pass=0;pass<3;pass++){
      const jitterScale = (1.5 + pass) * (2 + p.size);
      const offset = (pass-1)*1.5;
      ctx.globalAlpha = alpha * (0.9 - pass*0.25);
      ctx.strokeStyle = pass===0 ? '#ffffff' : p.color;
      ctx.lineWidth = Math.max(0.7, p.size*(1.2 - pass*0.35));
      ctx.beginPath();ctx.moveTo(p.x+nx*offset,p.y+ny*offset);
      for(let i=1;i<10;i++){
        const seg=i/10;
        const jitter=(i%2?1:-1)*jitterScale*Math.sin(seg*Math.PI+t+pass);
        ctx.lineTo(p.x+dx*seg+nx*(jitter+offset),p.y+dy*seg+ny*(jitter+offset));
      }
      ctx.lineTo(targetX+nx*offset,targetY+ny*offset);ctx.stroke();
    }
    // Bright core white
    line(0,Math.max(.5,p.size*.25),'#eaf0ff',1.0);
    // Endpoint spark
    if(G._fxTier<2){
      ctx.globalAlpha=alpha*.8;ctx.fillStyle=p.color;
      ctx.beginPath();ctx.arc(targetX,targetY,p.size*1.5,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#ffffff';ctx.globalAlpha=alpha*.9;
      ctx.beginPath();ctx.arc(targetX,targetY,p.size*.5,0,Math.PI*2);ctx.fill();
    }
  }else if(style==='prism'||style==='laser'||style==='phase'||style==='toxic'){
    // Prismatic laser with chromatic aberration and beam propagation
    line(0,p.size*7,p.color,0.06);
    line(0,p.size*4,p.color,0.12);
    line(-1.2,p.size*1.4,style==='toxic'?'#a3e635':'#ff3366',alpha*.55);
    line(1.2,p.size*1.4,style==='toxic'?'#4ade80':'#33aaff',alpha*.55);
    line(0,p.size+1,p.color,0.95);
    line(0,Math.max(.5,p.size*.22),'#ffffff',1.0);
    // Plasma ripple
    ctx.globalAlpha=alpha*0.18;
    ctx.strokeStyle=p.color;ctx.lineWidth=p.size*0.9;
    ctx.beginPath();ctx.moveTo(p.x,p.y);
    for(let i=1;i<=8;i++){const s=i/8;const w=Math.sin(s*Math.PI*5+t*2)*3;ctx.lineTo(p.x+dx*s+nx*w,p.y+dy*s+ny*w);}ctx.stroke();
    // Expanding beam tip spark
    if (progress < 1) {
      ctx.globalAlpha = alpha * 0.9;
      ctx.fillStyle = '#ffffff';
      ctx.beginPath(); ctx.arc(targetX, targetY, p.size * 2, 0, Math.PI * 2); ctx.fill();
    }
  }else if(style==='helix'){
    line(0,p.size*5,p.color,0.08);
    ...
    ctx.beginPath();ctx.moveTo(p.x,p.y);
    for(let i=1;i<=10;i++){const s=i/10;const wave=Math.sin(s*Math.PI*4+G.now*.02)*4;ctx.lineTo(p.x+dx*s+nx*wave,p.y+dy*s+ny*wave);}ctx.stroke();
  }else{
    line(0,p.size*4,p.color,0.1);line(0,p.size*2,p.color,0.2);line(0,p.size+1,p.color,.9);line(0,Math.max(.7,p.size*.28),'#ffffff',.88);
  }
  ctx.restore();
}
...
function drawProjectileGlyph(p, style, angle, size) {
  ...
  if (p.isBlackholeProj) {
    // Singularité - simplifiée sans particules (lag)
    const radius = p.size || 30; // Utilise p.size du projectile (affecté par les amplificateurs)
    const borderSize = radius * 0.2; // Bordure proportionnelle (20% du rayon)
    
    // Halo violet extérieur
    const bgGlow = ctx.createRadialGradient(0, 0, radius, 0, 0, radius * 3);
    bgGlow.addColorStop(0, 'rgba(75, 1, 80, 0.4)');
    bgGlow.addColorStop(0.5, 'rgba(128, 0, 128, 0.2)');
    ...
    // Lisière chaude rose/magenta autour du trou noir (proportionnelle)
    const innerGlow = ctx.createRadialGradient(0, 0, radius - borderSize * 0.5, 0, 0, radius + borderSize);
    innerGlow.addColorStop(0, '#000000');
    innerGlow.addColorStop(0.15, '#f15bb5');
    innerGlow.addColorStop(0.5, '#800080');
    innerGlow.addColorStop(1, 'rgba(15, 6, 22, 0)');
    ctx.globalAlpha = 0.9;
    ctx.fillStyle = innerGlow;
    ctx.beginPath();
    ctx.arc(0, 0, radius + borderSize, 0, Math.PI * 2);
    ctx.fill();
    
    // Cercle noir opaque au centre (Singularité)
    ctx.globalAlpha = 1;
    ctx.globalCompositeOperation = 'source-over';
    ctx.beginPath();
    ctx.arc(0, 0, radius, 0, Math.PI * 2);
    ctx.fillStyle = '#000000';
    ctx.shadowBlur = borderSize * 0.5; // Shadow proportionnelle
  ...
  if (p.isPiercing || p.isSonicWave || p.isRailgunProj) {
    // Sharp kinetic arrowhead / piercing spear
    ctx.fillStyle = p.color;
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    ctx.moveTo(size * 2.8, 0); // Sharp point
    ctx.lineTo(-size * 1.4, size * 0.85); // Right wing
    ctx.lineTo(-size * 0.6, 0); // Inner notch
    ctx.lineTo(-size * 1.4, -size * 0.85); // Left wing
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    // Hot white tip
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.arc(size * 1.8, 0, Math.max(1, size * 0.35), 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    return;
  }
