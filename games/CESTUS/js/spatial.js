...
function findClosestCore(x, y) {
  ...
  if (G.modules) {
    for (let i = 0; i < G.modules.length; i++) {
      const m = G.modules[i];
      if (!m.alive) continue;
      const def = typeof MODULE_TYPES !== 'undefined' ? MODULE_TYPES[m.typeId] : null;
      if (m.typeId === 'core' || (def && def.isSecondaryCore)) {
        const distSq = (m.x - x)*(m.x - x) + (m.y - y)*(m.y - y);
        if (distSq < minDistSq) {
          minDistSq = distSq;
          closest = m;
        }
  ...
  return closest || findClosestModule(x, y);
}
