...
function toggleGraphicsSettings(force) {
  ...
  if (typeof G !== 'undefined' && typeof gameStarted !== 'undefined' && gameStarted) {
    if (open && !G.paused) {
  ...
  document.body.insertAdjacentHTML('beforeend', `
    ...
    <div id="graphicsSettingsOverlay">
      <div class="graphics-settings-card">
        <div class="graphics-settings-head"><div><small>CESTUS // HELIOS</small><h2>PARAMÈTRES SYSTÈME</h2></div><button onclick="toggleGraphicsSettings(false)">✕</button></div>
        <div class="settings-mode-tabs"><button class="active" data-pane="render" onclick="showSettingsPane('render')">RENDU</button><button data-pane="balance" onclick="showSettingsPane('balance')">VAGUES & ÉCONOMIE</button></div>
        <div id="settingsPaneRender" class="settings-pane active">
        ...
        <div id="settingsPaneBalance" class="settings-pane">
          <div class="balance-settings-intro"><div><b>ÉQUILIBRAGE PERSONNALISÉ</b><span>Les valeurs par défaut correspondent au nouveau rythme longue durée.</span></div><button onclick="resetBalanceSettings()">VALEURS PAR DÉFAUT</button></div>
          <div id="balanceSettingsGrid"></div>
        </div>
