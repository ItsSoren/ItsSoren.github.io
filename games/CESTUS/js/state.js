...
function createGameState() {
  return {
    ...
    // Rift directives: one risk/reward contract per wave
    directiveOpen: false,
    directiveChoices: [],
    activeDirective: null,
    directiveHistory: [],
    directiveLastCleared: 0,
    directiveBatchSize: 1,
    riftStreak: 0,
    riftScore: 0,
    riftPortals: [],
    ...
    // Runtime-only performance caches
    liveEnemyCount: 0,
    _tabsDirty: false,
  };
}
...
function saveGame() {
  ...
  const cleanG = JSON.parse(JSON.stringify(G, function(key, value) {
    if (this === G) {
      const transient = ['target', 'lastRampTarget', 'enemies', 'projectiles', 'particles', 'floatingTexts', 'patrolUnits', 'corpses', 'liveEnemyCount', 'activeWaves', 'riftPortals', 'directiveOpen', 'directiveChoices', 'activeDirective', '_hudCache', 'cachedStats', '_enemyGrid', '_enemyById', '_moduleById', '_moduleGrid', '_moduleCellIndex', '_protectors', '_vp', '_gridGradient', '_gridGradientKey', '_vignetteGradient', '_vignetteGradientKey', '_particlePool', '_fxTier', '_fxRecoveryFrames', '_fxPressureFrames', 'fpsRealLast'];
