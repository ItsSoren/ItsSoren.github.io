...
function startGameAudioDrone() {
  const ac = GAME_AUDIO.ctx;
  if (!ac || ac.state !== 'running' || GAME_AUDIO.droneStarted) return;
  GAME_AUDIO.droneStarted = true;
...
function resumeGameAudio() {
  const ac = GAME_AUDIO.ctx;
  if (!ac || GAME_AUDIO.resumePending) return;
  if (ac.state === 'running') { startGameAudioDrone(); return; }
  if (navigator.userActivation && !navigator.userActivation.isActive) return;
  GAME_AUDIO.resumePending = true;
  Promise.resolve(ac.resume()).then(() => {
    GAME_AUDIO.resumePending = false;
    startGameAudioDrone();
  }).catch(() => { GAME_AUDIO.resumePending = false; });
}
...
function initGameAudio() {
  ...
  // Chrome blocks WebAudio created outside a real user gesture. Wave logic and
  // automated restores may call this function, so wait for a trusted action.
  if (navigator.userActivation && !navigator.userActivation.isActive) return;
...
window.addEventListener('click', initGameAudio, { once:true, capture:true });
